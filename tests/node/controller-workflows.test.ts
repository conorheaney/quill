import type { DocumentPort, FileState } from "../../frontend/scripts/contracts";

const assert = require("node:assert/strict");
const test = require("node:test");

const { createDocumentController } = require("../../frontend/scripts/document-controller.ts");
const {
  createControllerEvents,
  createFakeClock
} = require("./helpers/controller-fakes.ts");
...
function createDocumentPort(state: TestDocumentState): DocumentPort {
  return {
    getContent: () => state.content,
    setContent: (content: string) => { state.content = content; },
    getFileIdentity: () => ({
...
test("controller workflows apply successful load, save, and new-document results", async () => {
  ...
  const bridgeCalls: Array<{ operation: string; payload?: unknown }> = [];
  ...
  const controller = createDocumentController({
    desktopBridge: {
      ...
      async saveMarkdownFile(payload: unknown) {
...
test("rescheduling a draft clears the stale timer and persists only the latest content", () => {
  ...
  const controller = createDocumentController({
    ...
    storage: { saveDraft: (content: string) => persisted.push(content) }
  });
...
function createFileState(overrides: Partial<FileState> = {}): FileState {
  return {
    contentHash: "hash-1",
    exists: true,
    filePath: "C:/notes/current.md",
...
test("external changes can be reloaded without discarding the decision flow", async () => {
  ...
  const controller = createDocumentController({
    ...
    document: createDocumentPort(state),
    events: {
      onExternalReloaded(result: unknown) { reloads.push(result); },
