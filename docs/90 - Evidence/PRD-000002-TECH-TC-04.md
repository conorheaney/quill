...
- Keep the persistence suites and `tests/node/helpers/persistence-contract.ts` unchanged before execution.
...
3. Run `node --test tests/node/persistence-latest-edit-wins.test.ts tests/node/persistence-draft-recovery.test.ts`.
4. If the managed sandbox blocks worker spawning with `spawn EPERM`, run the same files with `node --test --test-isolation=none tests/node/persistence-latest-edit-wins.test.ts tests/node/persistence-draft-recovery.test.ts`.
...
| Isolated command | `node --test tests/node/persistence-latest-edit-wins.test.ts tests/node/persistence-draft-recovery.test.ts` blocked by sandbox `spawn EPERM`; 0 assertions executed |
| Executed fallback | `node --test --test-isolation=none tests/node/persistence-latest-edit-wins.test.ts tests/node/persistence-draft-recovery.test.ts` |
...
| Sources | [Latest-edit-wins suite](../../tests/node/persistence-latest-edit-wins.test.ts); [draft-recovery suite](../../tests/node/persistence-draft-recovery.test.ts); [persistence fakes](../../tests/node/helpers/persistence-contract.ts) |
